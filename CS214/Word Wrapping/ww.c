...
char * insertWrap(char * path, int size){
    ...
    //given a path such as "baz/sample.txt" we need to make it "baz/wrap.sample.txt"
    int i=0;//the index of the given path
    char * output = malloc((size+5)*sizeof(char));
    int j=0;//the index of the output being returned
    int num2=0; //number of times we encountered "/"
    if(num==0){
        //there are no "/" : sample.txt
        output[j++]='w';
        output[j++]='r';
        output[j++]='a';
        output[j++]='p';
...
void recursiveWrap(char * directory,char** dir_array,char**fil_array,int dir_len,int fil_len){
    ...
    if(directory!=NULL){
        ...
        //if directory pointer is NULL, that means entire array of directories is passed, possibly with entire file array

        if(dir_array!=NULL){
            for(int i=0;i<dir_len;i++){
                d_insert(dir_array[i],DirQueue);
            }
        }
        if(fil_array!=NULL){
            for(int i=0;i<fil_len;i++){
                f_insert(fil_array[i],insertWrap(fil_array[i],strlen(fil_array[i])+1),FilQueue);
            }
        }
    }
...
void multipleArguments(int argc, char ** argv){
    //this is the extra credit (20 points) portion where it word wrap takes in multiple arguments
    if(checkRecursive(argv)==0){
        //Recursive Activated With Multiple Arugments
        #undef BUFFER_SIZE
        ...
        char ** directories=NULL;int dir_len=0;
        char ** files=NULL;int fil_len=0;
        for(int i = 3 ; i<argc;i++){
            int length=strlen(argv[i]);
            char * arg = malloc(length+2);
            memcpy(arg,argv[i],length+1);
            if(checkDirectory(arg)!=0){
                DIR * dir=opendir(arg);
                if(dir){
                    closedir(dir);
                    directories= realloc(directories,(++dir_len)*sizeof(char*));
                    directories[dir_len-1]=arg;
                }else{
                    free(arg);
                }
            }else if(checkFile(arg)!=0 && checkTXT(arg,length)==0){
                int file_descriptor=open(arg,O_RDONLY);
                if(file_descriptor!=-1){
                    close(file_descriptor);
                    files=realloc(files,(++fil_len)*sizeof(char*));
                    files[fil_len-1]=arg;
                }else{
                    free(arg);
                }
            }else{
                //invalid argument: free and move on
                free(arg);
            }
        }
        //if we got some valid directories or files, we can pass them to be executed
        if(directories!=NULL || files!=NULL){
            //first argument NULL: sends a "signal" that we are not passing a single directory, rather arrays of files and/or directories
            recursiveWrap(NULL,directories,files,dir_len,fil_len);
        }
        if(directories==NULL && files == NULL){
            throwError(NULL);
        }
        ...
        //Multiple Arguments Without Recursive
        #undef BUFFER_SIZE
        #define BUFFER_SIZE atoi(argv[1])
        BUFFERSIZE=argv[1];
        HarmonyOS * localData= initialize(BUFFERSIZE);
        int directoriesfilesread=0;
        if(atoi(argv[1])==0){
            write(STDERR_FILENO,"Invalid Width\n",14);
            exit(EXIT_FAILURE);
        }
        for(int i = 0 ; i<argc-2;i++){
            ...
            char * arg = malloc(length+2);
            memcpy(arg,argv[i+2],length+1);
        
            if(checkDirectory(arg)!=0 ){
                //got a directory
                readFromDirectory(arg,localData);
                directoriesfilesread++;
            }else if(checkFile(arg)!=0 &&checkTXT(arg,length)==0){
                //got a file
                char * wrapFile=insertWrap(arg,strlen(arg)+1);
                readFromFileOutputToFile(arg,wrapFile,localData);
                free(wrapFile);
                directoriesfilesread++;
            }
            free(arg);
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        ...
        recursiveWrap(directory,NULL,NULL,0,0);
        atexit(exit_handler);
        if (status == FAILURE)
        {
            exit(EXIT_FAILURE);
        ...
        //Multiple Arguments
        //write(STDOUT_FILENO,"Extra Credit in Development\n",strlen("Extra Credit in Development\n")+1);
    ...
    //Normal non-recursive three arguments such as "./ww 80 directory" or "./ww 80 directory/text.txt"
    #undef BUFFER_SIZE
    #define BUFFER_SIZE atoi(argv[1])
    if(BUFFER_SIZE==0){
        write(STDERR_FILENO,"Invalid Width\n",14);
